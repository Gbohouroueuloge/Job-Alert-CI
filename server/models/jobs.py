...
from models.enums import IngestionAction, JobOfferStatus
...
class Company(UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin, Base):
    ...
    primary_filiere_id: Mapped[str | None] = mapped_column(
        ForeignKey("filieres.id", ondelete="SET NULL"), nullable=True
    )
...
class JobOffer(UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin, Base):
    ...
    slug: Mapped[str | None] = mapped_column(String(320), unique=True, nullable=True)

    company_id: Mapped[str] = mapped_column(
        ForeignKey("companies.id", ondelete="RESTRICT"), index=True, nullable=False
    )
    source_id: Mapped[str] = mapped_column(
        ForeignKey("sources.id", ondelete="RESTRICT"), index=True, nullable=False
    )
    location_id: Mapped[str | None] = mapped_column(
        ForeignKey("locations.id", ondelete="SET NULL"), index=True, nullable=True
    )
    primary_filiere_id: Mapped[str | None] = mapped_column(
        ForeignKey("filieres.id", ondelete="SET NULL"), index=True, nullable=True
    )
    specialty_id: Mapped[str | None] = mapped_column(
        ForeignKey("filiere_specialties.id", ondelete="SET NULL"), nullable=True
    )
    contract_type_id: Mapped[str | None] = mapped_column(
        ForeignKey("contract_types.id", ondelete="SET NULL"), index=True, nullable=True
    )
    experience_level_id: Mapped[str | None] = mapped_column(
        ForeignKey("experience_levels.id", ondelete="SET NULL"), index=True, nullable=True
    )
    education_level_id: Mapped[str | None] = mapped_column(
        ForeignKey("education_levels.id", ondelete="SET NULL"), index=True, nullable=True
    )
    ...
    detail: Mapped["JobOfferDetail | None"] = relationship(
        back_populates="offer", cascade="all, delete-orphan", uselist=False
    )
    filiere_links: Mapped[list["OfferFiliere"]] = relationship(
...
class OfferIngestionEvent(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    offer_id: Mapped[str | None] = mapped_column(ForeignKey("job_offers.id", ondelete="SET NULL"), index=True,
                                                 nullable=True)
    source_scrape_run_id: Mapped[str | None] = mapped_column(ForeignKey("source_scrape_runs.id", ondelete="SET NULL"),
